...
lead_time <- sapply(event_idx, function(e) {
  det <- drift_idx[drift_idx <= e]
  if (length(det) == 0) return(NA)
  e - max(det)
})
...
mean_lead <- mean(lead_time, na.rm = TRUE)
cat("Antecedência média (amostras):", mean_lead, "\n")
